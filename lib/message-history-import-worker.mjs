...
export function createMessageHistoryImportRepository(
  query,
  { storage = null, config = null, logger = console } = {},
) {
  if (typeof query !== "function") throw new TypeError("A database query function is required.");
  const local = storage?.historyImport ?? null;

  function requireLocal(jobOrGuild) {
    const guildId = String(jobOrGuild?.guildId ?? jobOrGuild ?? "");
    if (!local || !storage?.writeEnabled) {
      const error = new Error("History Import SQLite storage is unavailable.");
      error.code = "HISTORY_IMPORT_LOCAL_STORAGE_UNAVAILABLE";
      throw error;
    }
    if (!config?.sqliteFirstEnabled || !config?.isSqliteFirstGuild?.(guildId)) {
      const error = new Error("History Import SQLite-first is not enabled for this Guild.");
      error.code = "HISTORY_IMPORT_GUILD_NOT_ENABLED";
      throw error;
    }
    return local;
  }
  ...
  return Object.freeze({
    async recoverStale(stallSeconds) {
      ...
      `, [stallSeconds]);
      const recovered = rowsFromResult(result);
      for (const job of recovered) {
        if (local?.getJob(job.id)) {
          local.setJobState(job.id, "stalled", {
            retryState: job.version < 3
              ? "LEGACY_IMPORT_STALLED"
    ...
    async claimNext({ hostId, instanceId }) {
      const result = await query(`
        WITH candidate AS (
          ...
          WHERE "version" = 3 AND "status" = 'queued' AND "cancelRequested" = false
        ...
        RETURNING job.*
      `, [hostId, instanceId]);
      const job = rowsFromResult(result)[0] ?? null;
      if (job && config?.isSqliteFirstGuild?.(job.guildId)) {
        requireLocal(job).ensureJob({
          ...job,
          jobId: job.id,
          status: "preparing",
          lastHeartbeatAt: Date.now(),
          metadata: { schemaVersion: 3, controlPlane: "postgresql" },
        });
      }
      return job;
    },
    ...
    async prepareChannels(job, channels) {
      ...
      requireLocal(job).prepareJob(job, channels);
      ...
      const result = await query(`
        ...
        RETURNING *
      `, [job.id]);
      const updated = rowsFromResult(result)[0] ?? null;
      if (updated) {
        local.setJobState(job.id, updated.status, {
          currentChannelId: updated.currentChannelId ?? null,
          lastHeartbeatAt: Date.now(),
        });
      }
      return updated;
    ...
    async heartbeat(jobId) {
      const localJob = local?.getJob(jobId);
      if (localJob) {
        local.setJobState(jobId, localJob.status, { lastHeartbeatAt: Date.now() });
      }
      await query(`
        UPDATE "history_import_job"
      ...
      if (channel) {
        ...
        if (localJob) {
          local.ensureChannel({
            ...
            channelProgressId: channel.id,
            channelId: channel.channelId,
            channelName: channel.channelName,
            status: "running",
          });
          local.setJobState(jobId, "running", {
    ...
    async saveBatch({
      ...
      const localResult = requireLocal(guildId).saveBatch({
        jobId,
        guildId,
        channelProgressId,
        channelId,
      ...
      try {
        const result = await query(`
          WITH gate AS (
            ...
            SET "fetchedCount" = $3,
                "insertedCount" = $4,
                "duplicateCount" = $5,
                "nextBeforeMessageId" = $6,
                "oldestMessageId" = COALESCE($7, channel."oldestMessageId"),
          ...
          UPDATE "history_import_job" AS job
          SET "processedMessages" = $8,
              "fetchedMessages" = $9,
              "insertedMessages" = $10,
              "duplicateMessages" = $11,
              "lastApiResponseAt" = CURRENT_TIMESTAMP,
              "lastDbWriteAt" = CURRENT_TIMESTAMP,
              "lastProgressAt" = CURRENT_TIMESTAMP,
              "lastWorkerHeartbeatAt" = CURRENT_TIMESTAMP,
        ...
        `, [
          jobId,
          channelProgressId,
          localResult.channel.fetchedCount,
          localResult.channel.insertedCount,
          localResult.channel.duplicateCount,
          localResult.channel.nextBeforeMessageId,
          localResult.channel.oldestMessageId,
          localResult.job.eligibleCount,
          localResult.job.fetchedCount,
          localResult.job.insertedCount,
          localResult.job.duplicateCount,
        ]);
        cloudJob = rowsFromResult(result)[0] ?? null;
        cloudMetadataSynced = Boolean(cloudJob);
      } catch (error) {
        logger.warn?.("History Import Cloud metadata update deferred", {
          code: classifyImportError(error).code,
          jobId,
        });
      }
      return {
        ...(cloudJob ?? {}),
        fetchedMessages: localResult.job.fetchedCount,
        processedMessages: localResult.job.eligibleCount,
        insertedMessages: localResult.job.insertedCount,
        duplicateMessages: localResult.job.duplicateCount,
        batchEligible: localResult.eligibleCount,
        batchInserted: localResult.insertedCount,
        batchDuplicate: localResult.duplicateCount,
    ...
    async setRetry({ jobId, channelProgressId, retries, retryAfter, safe }) {
      const localChannel = local?.getChannelByProgressId(jobId, channelProgressId);
      if (localChannel) {
        local.setChannelState(jobId, localChannel.channelId, localChannel.status, {
          retryCount: retries,
          retryAfterAt: retryAfter,
        });
        local.setJobState(jobId, "running", {
          retryState: safe.code,
          retryAfterAt: retryAfter,
          lastHeartbeatAt: Date.now(),
        });
      }
    ...
    async settleChannel(jobId, channelProgressId, status, safe = null) {
      const localChannel = local?.getChannelByProgressId(jobId, channelProgressId);
      if (localChannel) {
        local.setChannelState(jobId, localChannel.channelId, status, {
          retryCount: 0,
          retryAfterAt: null,
    ...
    async pause(jobId, channelProgressId = null) {
      ...
      if (local?.getJob(jobId)) {
        local.setJobState(jobId, "paused", {
    ...
    async cancel(jobId, channelProgressId = null) {
      ...
      if (local?.getJob(jobId)) {
        local.setJobState(jobId, "cancelled", {
          currentChannelId: null,
          retryState: null,
          retryAfterAt: null,
          lastHeartbeatAt: Date.now(),
    ...
    async complete(jobId) {
      ...
      if (local?.getJob(jobId)) {
        local.setJobState(jobId, "completed", {
          currentChannelId: null,
          retryState: null,
          retryAfterAt: null,
          lastHeartbeatAt: Date.now(),
    ...
    async fail(jobId, safe) {
      if (local?.getJob(jobId)) {
        local.setJobState(jobId, "failed", {
          currentChannelId: null,
          retryState: safe.code,
          retryAfterAt: null,
          lastHeartbeatAt: Date.now(),
    ...
    async claimDeletionRequest(stallSeconds = 120) {
      const result = await query(`
        WITH candidate AS (
          SELECT request."id", request."guildId", request."actorId"
          FROM "message_import_audit_event" AS request
          WHERE request."eventType" = 'IMPORTED_HISTORY_DATA_DELETE_REQUESTED'
            AND NOT EXISTS (
              SELECT 1 FROM "message_import_audit_event" AS terminal
              WHERE terminal."eventType" IN (
                'IMPORTED_HISTORY_DATA_DELETED',
                'IMPORTED_HISTORY_DATA_DELETE_FAILED'
              )
                AND (terminal."counts" ->> 'requestId')::integer = request."id"
            )
            AND NOT EXISTS (
              SELECT 1 FROM "message_import_audit_event" AS started
              WHERE started."eventType" = 'IMPORTED_HISTORY_DATA_DELETE_STARTED'
                AND (started."counts" ->> 'requestId')::integer = request."id"
                AND started."createdAt" > CURRENT_TIMESTAMP - ($1 * INTERVAL '1 second')
            )
          ...
          INSERT INTO "message_import_audit_event" (
            "guildId", "eventType", "actorId", "counts"
          )
          SELECT candidate."guildId", 'IMPORTED_HISTORY_DATA_DELETE_STARTED',
                 candidate."actorId", jsonb_build_object('requestId', candidate."id")
          FROM candidate
          RETURNING "id"
        )
        SELECT candidate.* FROM candidate CROSS JOIN started
      `, [stallSeconds]);
      return rowsFromResult(result)[0] ?? null;
    },

    deleteImportedHistory({ requestId, guildId }) {
      return requireLocal(guildId).deleteImportedHistory({ requestId, guildId });
    },

    async completeDeletionRequest(request, result) {
      await query(`
        INSERT INTO "message_import_audit_event" (
          "guildId", "eventType", "actorId", "counts"
        ) VALUES (
          $1, 'IMPORTED_HISTORY_DATA_DELETED', $2,
          jsonb_build_object('requestId', $3::integer, 'deletedMessages', $4::integer)
        )
      `, [request.guildId, request.actorId ?? null, request.id, result.deletedMessages]);
    },

    async failDeletionRequest(request, safe) {
      await query(`
        INSERT INTO "message_import_audit_event" (
          "guildId", "eventType", "actorId", "counts", "safeErrorCode"
        ) VALUES (
          $1, 'IMPORTED_HISTORY_DATA_DELETE_FAILED', $2,
          jsonb_build_object('requestId', $3::integer), $4
        )
      `, [request.guildId, request.actorId ?? null, request.id, safe.code]);
    },
...
export function createMessageHistoryImportWorker({
      ...
      while (pages < config.maxPagesPerChannel) {
        ...
        await withBoundedImportRetry(
          () => repository.saveBatch({
            ...
            requestBeforeMessageId: before ?? null,
  ...
  async function processJob(job) {
    try {
      if (!config.sqliteFirstEnabled || !config.isSqliteFirstGuild?.(job.guildId)) {
        const safe = {
          code: "HISTORY_IMPORT_GUILD_NOT_ENABLED",
          summary: "このサーバーではSQLite-first履歴取り込みが有効ではありません。",
        };
        await repository.fail(job.id, safe);
        await audit({
          jobId: job.id,
          guildId: job.guildId,
  ...
  async function processDeletionRequest() {
    if (typeof repository.claimDeletionRequest !== "function") return null;
    const request = await repository.claimDeletionRequest(config.stallSeconds);
    if (!request) return null;
    if (!config.sqliteFirstEnabled || !config.isSqliteFirstGuild?.(request.guildId)) {
      const safe = {
        code: "HISTORY_IMPORT_GUILD_NOT_ENABLED",
        summary: "このサーバーではSQLite-first履歴取り込みが有効ではありません。",
      };
      await repository.failDeletionRequest(request, safe);
      return { requestId: request.id, failed: true };
    }
    try {
      const result = repository.deleteImportedHistory({
        requestId: request.id,
        guildId: request.guildId,
      });
      await repository.completeDeletionRequest(request, result);
      return { requestId: request.id, ...result };
    } catch (error) {
      const safe = classifyImportError(error);
      if (!safe.retryable) await repository.failDeletionRequest(request, safe);
      throw error;
    }
  }
  ...
  return Object.freeze({
    ...
    async poll() {
      ...
      processing = true;
      try {
        const deletion = await processDeletionRequest();
        if (deletion) return `delete:${deletion.requestId}`;
