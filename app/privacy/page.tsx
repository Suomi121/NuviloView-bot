      ...
      <section className="mx-auto max-w-3xl px-6 pb-24 pt-32">
        ...
        <div className="mt-10 space-y-8 text-sm leading-7 text-muted-foreground">
          ...
          <PolicySection title="5. 保存期間と削除">
            <p>検索用に保存するメッセージ本文は原則として最大90日間保存し、保存期間を過ぎたデータは順次削除します。履歴取り込みで取得した本文にも同じ保存期間を適用します。Botが削除イベントを受信したメッセージは保存データから削除しますが、zx?snipeで直近最大999,999件の削除を確認するため、削除本文と関連情報をBotメモリ上に最大90日間（約3か月）保持します。上限を超えた場合は古い履歴から切り捨て、この一時情報はNeonDBへ追加保存しません。認証情報、設定、統計、監査ログおよび問い合わせ情報は、機能提供、法令対応、不正利用防止または紛争対応に必要な期間保存します。</p>
