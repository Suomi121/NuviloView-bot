...
export function calculateHealthScore(input) {
  ...
  const messagesPerActivePerDay = messagesPerActive !== null && hasUsableDailyWindow
    ...
    : null;
  const reactionRate = input.reactionAvailable === false
    ? null
    : safeRate(input.reactions, input.messages);
  ...
  if (input.qualityGatePassed === false) availabilityReasons.push("data_quality_gate_failed");
