...
export function HeroSection() {
  ...
  return (
    ...
    >
      ...
      <div
        aria-hidden="true"
        className="pointer-events-none absolute left-1/2 top-[-10%] h-[520px] w-[520px] -translate-x-1/2 rounded-full bg-primary/25 blur-[140px]"
      />
      <div
        aria-hidden="true"
        className="pointer-events-none absolute bottom-[-20%] right-[-5%] h-[400px] w-[400px] rounded-full bg-primary/10 blur-[130px]"
      />
      {/* faint grid */}
      <div
        aria-hidden="true"
      ...
      <div className="relative z-10 mx-auto flex max-w-3xl flex-col items-center text-center">
        <div className="mb-8 inline-flex items-center gap-2 rounded-full border border-border bg-card/60 px-4 py-1.5 text-xs font-medium text-muted-foreground backdrop-blur-sm">
          <span className="h-1.5 w-1.5 rounded-full bg-primary" />
          分析 × OEM ダッシュボード
        </div>

        <h1 className="text-balance text-4xl font-extrabold leading-[1.15] tracking-tight text-foreground sm:text-5xl md:text-6xl">
          あなたのDiscordコミュニティを、
        ...
        <div className="mt-10 w-full max-w-xl rounded-2xl border border-border/80 bg-card/55 p-4 text-left shadow-2xl shadow-black/10 backdrop-blur-xl sm:p-5">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="mt-1 text-xs leading-relaxed text-muted-foreground">メッセージ数とメンバー推移を、安全に自動集計します。</p>
            </div>
            {botInviteUrl ? (
              <a
                href={botInviteUrl}
          ...
          <div className="mt-4 flex flex-wrap gap-x-4 gap-y-2 border-t border-border/70 pt-3 text-[11px] text-muted-foreground">
            <span className="flex items-center gap-1.5"><Check className="h-3.5 w-3.5 text-primary" />検索用にメッセージ本文を保存します</span>
            <span className="flex items-center gap-1.5"><Check className="h-3.5 w-3.5 text-primary" />いつでもサーバーから削除可能</span>
          </div>
        </div>
        ...
        <section aria-labelledby="analysis-examples" className="mt-16 w-full max-w-5xl text-left">
          ...
          <div className="grid gap-3 sm:grid-cols-3">
            <article className="rounded-2xl border border-border/80 bg-card/50 p-5 backdrop-blur-sm">
            ...
            <article className="rounded-2xl border border-border/80 bg-card/50 p-5 backdrop-blur-sm">
              ...
              <p className="mt-2 text-sm leading-relaxed text-muted-foreground">送信メッセージ、アクティブメンバー、リアクション率の推移を追えます。</p>
            </article>
            <article className="rounded-2xl border border-border/80 bg-card/50 p-5 backdrop-blur-sm">
              <Zap className="h-5 w-5 text-primary" />
              <h3 className="mt-4 font-bold">通話の盛り上がり</h3>
              <p className="mt-2 text-sm leading-relaxed text-muted-foreground">サーバー内で通話が行われた合計時間と、最長連続通話時間を記録します。</p>
            </article>
          </div>
