...
function GuildRow({ guild, onBlock, onUnblock }: { guild: ManagedGuild; onBlock: () => void; onUnblock: () => void }) {
  const blocked = Boolean(guild.reason)
  return <article className="flex flex-col gap-3 rounded-xl border border-border bg-background/45 p-3.5 sm:flex-row sm:items-center"><div className="flex min-w-0 flex-1 items-center gap-3">{guild.iconUrl ? <img src={guild.iconUrl} alt="" className="h-10 w-10 rounded-xl object-cover" /> : <span className="flex h-10 w-10 items-center justify-center rounded-xl bg-primary/15 text-primary"><Server className="h-5 w-5" /></span>}<div className="min-w-0"><p className="truncate text-sm font-bold">{guild.name || `Guild ${guild.guildId}`}</p><p className="mt-0.5 font-mono text-[11px] text-muted-foreground">{guild.guildId} · {guild.memberCount?.toLocaleString() ?? '—'} members</p><p className="mt-1 text-[11px] text-muted-foreground">{blocked ? `理由: ${guild.reason}` : guild.isConnected ? `接続中 · 最終確認 ${formatDate(guild.lastSeenAt)}` : 'Botは現在このGuildにいません'}</p></div></div><div className="flex flex-wrap items-center gap-2"><span className={`rounded-full px-2.5 py-1 text-[11px] font-bold ${blocked ? 'bg-rose-500/15 text-rose-400' : guild.isConnected ? 'bg-emerald-500/15 text-emerald-400' : 'bg-muted text-muted-foreground'}`}>{blocked ? 'BLOCKED' : guild.isConnected ? 'CONNECTED' : 'OFFLINE'}</span>{!blocked && guild.isConnected && <a href={`/developer/guilds/${guild.guildId}/reset`} className="inline-flex items-center gap-1.5 rounded-lg border border-primary/30 px-3 py-2 text-xs font-bold text-primary hover:bg-primary/10"><RefreshCcw className="h-3.5 w-3.5" />初期化</a>}{blocked ? <button onClick={onUnblock} className="inline-flex items-center gap-1.5 rounded-lg border border-emerald-500/30 px-3 py-2 text-xs font-bold text-emerald-400 hover:bg-emerald-500/10"><Unlock className="h-3.5 w-3.5" />解除</button> : <button onClick={onBlock} className="inline-flex items-center gap-1.5 rounded-lg border border-rose-500/30 px-3 py-2 text-xs font-bold text-rose-400 hover:bg-rose-500/10"><Ban className="h-3.5 w-3.5" />ブロック</button>}</div></article>
}
