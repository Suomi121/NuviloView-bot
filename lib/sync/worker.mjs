...
export class SyncWorker {
  ...
  async #writeSnapshot(guards, { force = false } = {}) {
    ...
    Object.assign(snapshot, this.storage.messageDomain?.getMetrics?.() ?? {});
    const at = this.now();
    if (
      !force &&
  ...
  async #processBatch(batch) {
    ...
    try {
      ...
      this.storage.messageDomain?.recordSyncResult?.({
        failureCount: batch.filter((item) => item.eventType.startsWith("message_")).length,
        at: this.now(),
      });
    ...
    const messageSucceeded = batch.filter(
      (item) => succeeded.has(item.eventId) && item.eventType.startsWith("message_"),
    ).length;
    const messageFailed = batch.filter(
      (item) => !succeeded.has(item.eventId) && item.eventType.startsWith("message_"),
    ).length;
    this.storage.messageDomain?.recordSyncResult?.({
      successCount: messageSucceeded,
      failureCount: messageFailed,
      at: this.now(),
    });
