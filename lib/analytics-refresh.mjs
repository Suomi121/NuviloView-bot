...
export function createAnalyticsRefreshContract(
  ...
  const normalizedIntervalMs = Math.min(
    MAXIMUM_ANALYTICS_REFRESH_INTERVAL_MS,
    Math.max(
  ...
  const configuredNextUpdateAt = positiveTimestamp(
    metadata?.nextUpdateAt ?? metadata?.next_update_at,
  );
  const baseNextUpdateAt = configuredNextUpdateAt ?? at + normalizedIntervalMs;
  const nextUpdateAt = baseNextUpdateAt > at
    ? baseNextUpdateAt
    : baseNextUpdateAt
      + (Math.floor((at - baseNextUpdateAt) / normalizedIntervalMs) + 1)
...
export function toAnalyticsRefreshSchedule(value) {
  ...
  const intervalMs = Math.min(
    ...
    Math.max(
      MINIMUM_ANALYTICS_REFRESH_INTERVAL_MS,
      requestedInterval ?? DEFAULT_ANALYTICS_REFRESH_INTERVAL_MS,
    ),
  );
