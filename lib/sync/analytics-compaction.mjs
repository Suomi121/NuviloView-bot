...
export function getAnalyticsCompactionConfig(env = process.env) {
  ...
  const errors = [];
  if (enabled && configuredGuildIds.length === 0) {
  ...
  return Object.freeze({
    ...
    isEnabledForGuild: (guildId) =>
      enabled && configuredGuildIds.includes(String(guildId ?? "")),
  });
}
...
export function createAnalyticsCompactionService(
  ...
  function bootstrapStreamName(guildId) {
    return `analytics_compaction_bootstrap_v2:${guildId}`;
  }
  ...
  function bootstrap() {
    ...
    bootstrapped = true;
    const pendingGuildIds = config.guildIds.filter(
      (guildId) =>
    ...
    for (const guildId of pendingGuildIds) {
      storage.syncMetadata.set({
        streamName: bootstrapStreamName(guildId),
        state: "complete",
        lastAttemptAt: at,
        lastSuccessAt: at,
  ...
  function refreshDue({ at = now(), limit = config.batchSize } = {}) {
    if (!config.enabled || config.guildIds.length === 0) {
      return {
