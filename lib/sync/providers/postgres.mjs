...
export function createPostgresProviderAdapter({
  ...
  async function getSnapshotStates(items) {
    ...
    for (const [snapshotType, aggregateIds] of groups) {
      const tableName = snapshotTableByType[snapshotType];
      const result = await execute(
        `SELECT aggregate_id, snapshot_version, checksum
         FROM ${tableName} WHERE aggregate_id = ANY($1::text[])`,
        [[...new Set(aggregateIds)]],
      );
      for (const row of resultRows(result)) {
        states.set(`${snapshotType}:${String(row.aggregate_id)}`, {
  ...
  async function listSnapshots({
    ...
    const tableName = snapshotTableByType[String(snapshotType)];
    if (!tableName) throw new TypeError("Unsupported snapshot type.");
    const normalizedPrefix = String(aggregateIdPrefix ?? "");
    const normalizedLimit = Math.min(50_000, Math.max(1, Number(limit) || 10_000));
    const result = await execute(
      `SELECT aggregate_id, snapshot_version, payload, checksum,
              generated_at, synced_at
       FROM ${tableName}
       WHERE aggregate_id >= $1 AND aggregate_id < $2
         AND ($3::text IS NULL OR payload->>'dateUtc' >= $3)
         AND ($4::text IS NULL OR payload->>'dateUtc' <= $4)
       ORDER BY aggregate_id
       LIMIT $5`,
      [normalizedPrefix, `${normalizedPrefix}\uffff`, dateFrom, dateTo, normalizedLimit],
    );
