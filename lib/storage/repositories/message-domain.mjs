...
export function createMessageDomainRepository(
  ...
  function recordEvent(input) {
    ...
    const inserted = store.run(
      `INSERT OR IGNORE INTO message_event_log (
    ...
    if (!sameStoredEvent(storedRow, normalized)) {
    ...
    if (Number(inserted.changes) === 1) {
      const currentWrite = store.run(
        `INSERT INTO message_events (
           ...
           content_checksum, delete_event_id
         ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
         ...
         WHERE excluded.source_sequence > message_events.source_sequence
