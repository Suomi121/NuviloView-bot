...
The global flag and an explicit Guild allowlist are both required. Every Event
Canary Guild must also be listed in `ANALYTICS_COMPACTION_GUILD_IDS`, and local
storage, local writes, snapshot sync, the Sync Worker, and Analytics Compaction
must already be enabled. The default configuration preserves the legacy path
only as a staged-rollout/rollback boundary. A Guild listed in the Event
allowlist never dual-writes Reaction, Voice, or Member raw rows to Cloud.
...
currently supported Guilds to be moved into the verified allowlist; this
fallback is not a dual-write path.
