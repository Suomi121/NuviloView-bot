...
const connectionStringSslParameters = Object.freeze([
  "sslmode",
  "sslcert",
  "sslkey",
  "sslrootcert",
...
export function createPostgresPoolConfig(
  ...
  if (!caCertificate) return poolConfig;

  // pg-connection-string reads sslcert/sslkey/sslrootcert paths while parsing
  // a connection URL. A path valid on Termux is not present in Vercel, so use
  // the explicitly supplied CA contents and preserve full certificate checks.
...
function createPool(connectionString, config, applicationName, caCertificate = null) {
  return new Pool(
