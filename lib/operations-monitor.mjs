...
export function getOperationsMonitorConfig(environment = process.env) {
  ...
  return Object.freeze({
    ...
    repeatedHighThreshold: integer(
      environment,
      "NUVILOVIEW_MONITOR_SECURITY_HIGH_THRESHOLD",
      3,
      2,
      100,
    ),
...
export function evaluateOperationsSnapshot({
  ...
  if (Number(security.openCritical) > 0) {
    addIncident(incidents, "CRITICAL", "security_critical_incident", `${security.openCritical} open Critical security incident(s) require review.`);
  }
  if (Number(security.recentHigh) >= config.repeatedHighThreshold) {
    addIncident(incidents, "WARNING", "security_repeated_high", `${security.recentHigh} High security incidents occurred in the monitoring window.`);
  }
