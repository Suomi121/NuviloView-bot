...
export function createOutboxRepository(
  ...
  function enqueue(input) {
    ...
    return inTransaction(() => {
      ...
      providerDeliveries?.ensureForEvent?.(
        envelope.eventId,
        providerDefinitions,
  ...
  function getMessagePendingCount({ guildId = null } = {}) {
    const normalizedGuildId = guildId == null ? null : requireString(guildId, "guildId");
    return Number(
      store.get(
        `SELECT COUNT(*) AS count FROM sync_outbox
         WHERE event_type LIKE 'message_%'
           AND status IN ('pending', 'retry', 'processing')
           ${normalizedGuildId === null ? "" : "AND json_extract(payload_json, '$.guildId') = ?"}`,
        ...(normalizedGuildId === null ? [] : [normalizedGuildId]),
      )?.count ?? 0,
    );
  }

  function getMessageOldestPendingAge({ guildId = null, at = now() } = {}) {
    const normalizedGuildId = guildId == null ? null : requireString(guildId, "guildId");
    const row = store.get(
  ...
  function getMessageDeadLetterCount({ guildId = null } = {}) {
    const normalizedGuildId = guildId == null ? null : requireString(guildId, "guildId");
    return Number(
      store.get(
  ...
  function getMessageSyncStatus({ guildId = null, at = now() } = {}) {
    const normalizedGuildId = guildId == null ? null : requireString(guildId, "guildId");
    const row = store.get(
      `SELECT
         MAX(CASE WHEN status = 'synced'
           THEN CAST(json_extract(payload_json, '$.occurredAt') AS INTEGER) END) AS last_synced_message_at,
         MAX(CAST(json_extract(payload_json, '$.occurredAt') AS INTEGER)) AS last_local_message_at,
         MIN(CASE WHEN status IN ('pending', 'retry', 'processing')
           THEN created_at END) AS oldest_pending_at,
         SUM(CASE WHEN status IN ('pending', 'retry', 'processing') THEN 1 ELSE 0 END) AS pending_count
       FROM sync_outbox
       WHERE event_type LIKE 'message_%'
       ${normalizedGuildId === null ? "" : "AND json_extract(payload_json, '$.guildId') = ?"}`,
      ...(normalizedGuildId === null ? [] : [normalizedGuildId]),
    ...
    return {
      lastSyncedMessageAt: row?.last_synced_message_at == null
        ? null
        : Number(row.last_synced_message_at),
      lastLocalMessageAt: row?.last_local_message_at == null
        ? null
        : Number(row.last_local_message_at),
      oldestPendingAgeMs: row?.oldest_pending_at == null
        ? null
        : Math.max(0, at - Number(row.oldest_pending_at)),
  ...
  function listForReconciliation({ limit = 100, afterCreatedAt = null } = {}) {
    if (!Number.isInteger(limit) || limit < 1 || limit > 10_000) {
      throw new TypeError("limit must be an integer between 1 and 10000.");
    }
    const rows = store.all(
      `SELECT event_id, checksum, created_at
       FROM sync_outbox
