...
export function getEventLocalFirstConfig(
  ...
  const errors = [];
  if (enabled && guildIds.length === 0) errors.push("event_local_first_guild_list_empty");
  ...
  for (const guildId of guildIds) {
    if (enabled && !compaction.guildIds.includes(guildId)) {
  ...
  return Object.freeze({
    ...
    isEnabledForGuild(domain, guildId) {
      ...
      return Boolean(
        ...
        guildIds.includes(String(guildId ?? "")),
      );
    },
  });
}
...
export function createEventDomainRouter({
  ...
  return Object.freeze({
    enabled: config.enabled && config.guildIds.length > 0,
    config,
    storage,
    reaction,
    voice,
