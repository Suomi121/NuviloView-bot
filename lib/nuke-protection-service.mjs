...
export function createNukeProtectionService({ client, sql, environment = process.env, logger = console }) {
  ...
  async function loadPolicy(guildId) {
    ...
    if (!rows[0]) {
      await sql`
        INSERT INTO "security_policy" ("guildId", "enabled", "mode", "createdAt", "updatedAt")
        VALUES (${guildId}, true, 'shadow', now(), now())
  ...
  async function diagnoseGuild(guild) {
    ...
    try {
      ...
      missingPermissions = [...permissionChecks]
        ...
        .map(([name]) => name);
      if (!globallyEnabled || !policy.enabled) {
        status = "Disabled";
        reason = globallyEnabled ? "Guild policy is disabled" : "NUVILOVIEW_NUKE_PROTECTION is disabled";
      } else if (!hasGatewayIntent) {
        status = "Limited";
  ...
  async function handleWebhookUpdate(channel) {
    ...
    for (const delayMs of [0, 350, 900]) {
      ...
      try {
        ...
        if (entry) return handleAuditLogEntry(entry, guild);
      } catch (error) {
  ...
  async function handleBotMessage(message) {
    ...
    try {
      const policy = await loadPolicy(guild.id);
      if (!policy.enabled || !policy.botSpamProtection) return null;
  ...
  async function createSnapshotForGuild(guild, { source = "manual", createdBy = null } = {}) {
    const policy = await loadPolicy(guild.id);
    if (!globallyEnabled || !policy.enabled || !policy.snapshotEnabled) {
  ...
  async function ensureDailySnapshot(guild) {
    ...
    const policy = await loadPolicy(guild.id);
    if (!policy.enabled || !policy.snapshotEnabled) return null;
  ...
  async function containIncident(guild, request) {
    const policy = await loadPolicy(guild.id);
    if (policy.mode === "shadow") throw Object.assign(new Error("Shadow mode does not permit containment."), { code: "SHADOW_MODE" });
