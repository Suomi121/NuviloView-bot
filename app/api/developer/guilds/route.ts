...
import { getGuildResetConfig } from '@/lib/guild-reset-utils.mjs'
...
async function purgeGuildData(guildId: string) {
  await Promise.all([
    ...
    pool.query('DELETE FROM "reaction_role_rule" WHERE "guildId" = $1', [guildId]),
  ])
}
...
export async function GET(request: Request) {
  ...
  try {
    const includeAudit = new URL(request.url).searchParams.get('includeAudit') !== 'false'
    ...
    const chainedAudit = includeAudit
      ? [...audit.rows].reverse().filter((entry) => entry.entryHash)
      : []
