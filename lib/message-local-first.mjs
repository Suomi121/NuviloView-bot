...
export function verifyMessageRoutingSafety({
  ...
  try {
    ...
    const legacyGlobalState = routing?.state === "local_first" && previousGuildIds.length === 0;
    ...
    if (guard.storage.writeEnabled && pendingCount === 0) {
      ...
      guard.storage.syncMetadata.set({
        ...
        state: activeGuildIds.length > 0 ? "canary" : "legacy_neon",
        metadata: { schemaVersion: 2, canaryGuildIds: activeGuildIds },
      });
      ...
      if (activeGuildIds.length > 0 && canarySetChanged) {
    ...
    return {
      mode: activeGuildIds.length > 0 ? "canary" : "legacy_neon",
      pendingCount,
      canaryGuildIds: activeGuildIds,
    };
  } finally {
...
export function createMessageDomainRouter({
  ...
  if (
    compaction.enabled &&
    (compaction.errors.length > 0 || compactionOutsideCanary.length > 0)
  ) {
    const error = new Error(
  ...
  return Object.freeze({
    enabled: config.enabled && config.canaryGuildIds.length > 0,
    globalEnabled: config.enabled,
    config,
    compaction,
    routing,
