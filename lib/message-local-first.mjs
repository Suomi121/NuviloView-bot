...
export function getMessageLocalFirstConfig(
  ...
  return Object.freeze({
    enabled: enabled(env.LOCAL_MESSAGE_STORAGE_ENABLED),
    forceLegacyWithPending: enabled(
      env.LOCAL_MESSAGE_STORAGE_FORCE_LEGACY_WITH_PENDING,
    ),
...
export function verifyMessageRoutingSafety({
  ...
  if (config.enabled) {
    ...
    return { mode: "local_first", pendingCount: storage.outbox.getMessagePendingCount() };
  }

  const guard = openGuardStorage(config, storage);
  if (!guard.storage) return { mode: "legacy_neon", pendingCount: 0 };
  try {
    const routing = guard.storage.syncMetadata.get("message_domain_routing");
    const pendingCount = guard.storage.outbox.getMessagePendingCount();
    if (
      routing?.state === "local_first" &&
      pendingCount > 0 &&
    ...
    return { mode: "legacy_neon", pendingCount };
  } finally {
...
export function createMessageDomainRouter({
  ...
  function persistOne(message, eventType) {
    ...
    return storage.transaction(() => {
      ...
      const eventInput = existingEvent ?? normalized;
  ...
  async function create(message) {
    if (!config.enabled) return legacy.create(message);
    try {
  ...
  async function update(message) {
    if (!config.enabled) return legacy.update(message);
    try {
  ...
  async function remove(message) {
    if (!config.enabled) return legacy.remove(message);
    try {
    ...
    try {
      return storage.transaction(() =>
        messages.map((message) => {
          ...
          const existingEvent = storage.messageDomain.getEventById(normalized.eventId);
          const result = storage.messageDomain.recordEvent(existingEvent ?? normalized);
          storage.outbox.enqueue(messageEnvelope(result.event));
          return { ...result, previous };
        }),
      );
