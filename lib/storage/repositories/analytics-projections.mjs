...
// Projection keys remain stable across contract revisions.
...
export function createAnalyticsProjectionRepository(
  ...
  function markAggregated(
    projectionKey,
    sourceSequence,
    { at = now(), intervalMs, nextEligibleAt = null },
  ) {
    const interval = Number(intervalMs);
    if (!Number.isSafeInteger(interval) || interval < 1) {
      throw new TypeError("intervalMs must be a positive integer.");
    }
    const eligibleAt = nextEligibleAt == null ? at + interval : Number(nextEligibleAt);
    if (!Number.isSafeInteger(eligibleAt) || eligibleAt <= at) {
      throw new TypeError("nextEligibleAt must be an integer after at.");
    }
  ...
  function buildMaterial(item) {
    ...
    const messageOperations = store.get(
      `SELECT
         SUM(CASE WHEN event_type = 'create' THEN 1 ELSE 0 END) AS creates,
         SUM(CASE WHEN event_type = 'update' THEN 1 ELSE 0 END) AS edits,
       ...
       FROM message_event_log m WHERE ${messageScope.sql}`,
      ...messageScope.parameters,
    );
    ...
    return {
      ...
      messageCreates: Number(messageOperations?.creates ?? 0),
      messageEdits: Number(messageOperations?.edits ?? 0),
      messageDeletes: Number(messageOperations?.deletes ?? 0),
      messageReplies: Number(messageOperations?.replies ?? 0),
  ...
  function listBackfillCandidates({ guildId, startAt, endAt, maxBuckets = 250 }) {
    const normalizedGuildId = requireString(guildId, "guildId");
    const normalizedStartAt = toEpochMilliseconds(startAt, "startAt");
    const normalizedEndAt = toEpochMilliseconds(endAt, "endAt");
    if (normalizedEndAt <= normalizedStartAt) {
      throw new TypeError("endAt must be after startAt.");
    }
    if (!Number.isInteger(maxBuckets) || maxBuckets < 1 || maxBuckets > 10_000) {
      throw new TypeError("maxBuckets must be between 1 and 10000.");
    }
    const parameters = [];
    const source = (table, channel, user) => {
      parameters.push(normalizedGuildId, normalizedStartAt, normalizedEndAt);
      return `SELECT guild_id, ${channel} AS channel_id, ${user} AS user_id,
                     occurred_at AS source_sequence
              FROM ${table}
              WHERE guild_id = ? AND occurred_at >= ? AND occurred_at < ?`;
    };
    const rawSources = [
      source("message_event_log", "channel_id", "author_id"),
      source("reaction_events", "channel_id", "user_id"),
      source("voice_events", "channel_id", "user_id"),
      source("voice_events", "previous_channel_id", "user_id"),
      source("member_events", "NULL", "user_id"),
    ];
    const rows = store.all(
      `WITH raw AS (${rawSources.join(" UNION ALL ")}), candidates AS (
         SELECT 'guild_current' AS projection_kind, guild_id,
                NULL AS date_utc, NULL AS channel_id, NULL AS user_id,
                MAX(source_sequence) AS source_sequence
         FROM raw GROUP BY guild_id
         UNION
         SELECT 'guild_daily', guild_id,
                date(source_sequence / 1000, 'unixepoch'), NULL, NULL,
                MAX(source_sequence)
         FROM raw GROUP BY guild_id, date(source_sequence / 1000, 'unixepoch')
         UNION
         SELECT 'channel_daily', guild_id,
                date(source_sequence / 1000, 'unixepoch'), channel_id, NULL,
                MAX(source_sequence)
         FROM raw WHERE channel_id IS NOT NULL
         GROUP BY guild_id, channel_id, date(source_sequence / 1000, 'unixepoch')
         UNION
         SELECT 'user_daily', guild_id,
                date(source_sequence / 1000, 'unixepoch'), NULL, user_id,
                MAX(source_sequence)
         FROM raw WHERE user_id IS NOT NULL
         GROUP BY guild_id, user_id, date(source_sequence / 1000, 'unixepoch')
       )
       SELECT * FROM candidates
       ORDER BY date_utc, projection_kind, channel_id, user_id
       LIMIT ?`,
      ...parameters,
      maxBuckets + 1,
    );
    ...
    return {
      ...
      maxBuckets,
      truncated,
      candidates: rows.slice(0, maxBuckets).map((row) => ({
        projectionKind: row.projection_kind,
        guildId: row.guild_id,
        dateUtc: row.date_utc,
        channelId: row.channel_id,
        userId: row.user_id,
        sourceSequence: Number(row.source_sequence),
        lastEventAt: Number(row.source_sequence),
      })),
    };
  }

  function markBackfillCandidates(candidates, { at = now() } = {}) {
    if (!Array.isArray(candidates)) throw new TypeError("candidates must be an array.");
    return inTransaction(() => {
      const keys = new Set();
      for (const candidate of candidates) {
        keys.add(markDirty(candidate, { at }).projectionKey);
      }
      return { marked: keys.size, projectionKeys: [...keys] };
    });
  }
  ...
  function getMetrics() {
    ...
    const bucketCounts = store.get(
      `SELECT COUNT(*) AS tracked,
              SUM(CASE WHEN dirty = 1 THEN 1 ELSE 0 END) AS dirty,
              MAX(last_aggregated_sequence) AS checkpoint
       FROM analytics_projection_dirty`,
    );
    return {
      rawEventsSeen,
      ...
      bucketsTracked: Number(bucketCounts?.tracked ?? 0),
      bucketsDirty: Number(bucketCounts?.dirty ?? 0),
      sourceCheckpoint:
        bucketCounts?.checkpoint == null ? null : Number(bucketCounts.checkpoint),
      requiredProviderWriteSkips: snapshotsSkippedChecksum * 2,
