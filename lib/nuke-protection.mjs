...
export const NUKE_PROTECTION_MODES = Object.freeze(["off", "shadow", "active"]);
...
export function normalizeNukeProtectionMode(value) {
  const normalized = String(value ?? "").trim().toLowerCase();
  return NUKE_PROTECTION_MODES.includes(normalized) ? normalized : "shadow";
}

export function resolveNukeProtectionMode({ globallyEnabled, guildEnabled = true, mode } = {}) {
  if (globallyEnabled !== true || guildEnabled !== true) return "off";
  return normalizeNukeProtectionMode(mode);
}
...
export function normalizeNukeProtectionPolicy(input = {}) {
  ...
  return {
    ...
    nukeProtectionMode: normalizeNukeProtectionMode(input.nukeProtectionMode),
    mode,
    sensitivity,
...
export function calculateNukeRisk(actions, options = {}) {
  ...
  const correlationConfidence = ["low", "medium", "high"].includes(options.correlationConfidence)
    ? options.correlationConfidence
    : "high";
  let riskScore = suppressed ? 0 : Math.min(100, rawRisk);
  // A delayed lookup or missing actor must never be promoted to Critical solely
  // by burst scoring. High-confidence Gateway audit entries keep existing behavior.
  if (!suppressed && correlationConfidence === "medium") {
    riskScore = Math.min(riskScore, Math.max(0, policy.thresholds.critical - 1));
  } else if (!suppressed && correlationConfidence === "low") {
    riskScore = Math.min(riskScore, Math.max(0, policy.thresholds.suspicious - 1));
  }
  return {
    riskScore,
    rawRisk,
