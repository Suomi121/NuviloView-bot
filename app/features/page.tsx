...
const features = [
  { icon: ChartNoAxesCombined, title: '日次アナリティクス', body: '総メンバー数、メッセージ数、リアクション率を日ごとに集計し、推移グラフで確認できます。' },
  { icon: Users, title: 'アクティブメンバー', body: '当日に発言したユニークユーザー数を計測。コミュニティの実際の活発さを把握できます。' },
  { icon: Activity, title: '最近のアクティビティ', body: 'メッセージ送信・参加・退出を時系列で表示します。検索機能を有効にしたサーバーでは、メッセージ本文も保存対象になります。' },
  { icon: Sparkles, title: '成長インサイト', body: '直近7日と前週の実績を比較し、メンバーと会話量の変化を自動で要約します。' },
  { icon: LockKeyhole, title: '権限ベースの閲覧', body: 'Discordで管理権限を持つサーバーだけを表示。サーバーデータを他の利用者に公開しません。' },
  { icon: Bot, title: '軽量な分析Bot', body: 'Botはメンバー数・件数・リアクション・通話時間を集計。必要な場合だけ検索用にメッセージ本文を保存します。' },
]
